...
def evaluate_predictions(
    ...
    metrics = {
        ...
        'exact_match': (total_em / num_questions) * 100,  # Convert to percentage
        'f1_score': (total_f1 / num_questions) * 100,      # Convert to percentage
        
        # Retrieval quality
        'recall@10': (total_recall_at_10 / num_questions) * 100 if compute_retrieval_metrics else None,
        
        # Latency metrics
        'avg_retrieve_time_ms': total_retrieve_time / num_questions,
        'avg_generate_time_ms': total_generate_time / num_questions,
        'avg_total_time_ms': total_time / num_questions,
...
def print_metrics(metrics: Dict, baseline_name: str = "Baseline"):
    ...
    print(f"\n📈 Questions Evaluated: {metrics['num_questions']}")
    print("=" * 70)
...
if __name__ == "__main__":
    ...
    print(f"  'Apple' vs 'apple': {exact_match('Apple', 'apple')}")  # 1.0
    print(f"  'The Apple Inc.' vs 'apple inc': {exact_match('The Apple Inc.', 'apple inc')}")  # 1.0
    print(f"  'Apple' vs 'Microsoft': {exact_match('Apple', 'Microsoft')}")  # 0.0
    
    # Test F1 score
    print("\nTesting F1 Score:")
    print(f"  'revenue was 394B' vs 'revenue 394B usd': {f1_score('revenue was 394B', 'revenue 394B usd'):.3f}")
    print(f"  'Apple Inc' vs 'Microsoft Corp': {f1_score('Apple Inc', 'Microsoft Corp'):.3f}")
    ...
    print(f"  Recall@10: {recall_at_k(retrieved, ground_truth, k=10):.3f}")  # 0.5 (1 out of 2)
